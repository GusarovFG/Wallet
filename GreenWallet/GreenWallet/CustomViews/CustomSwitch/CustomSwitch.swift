...
public class CustomSwitch: UIControl {
    ...
    @IBInspectable  public var onTintColor: UIColor = #colorLiteral(red: 0.2274509804, green: 0.6745098039, blue: 0.3490196078, alpha: 1) {
        didSet {
            self.setupUI()
        }
    }
    
    @IBInspectable public var offTintColor: UIColor = #colorLiteral(red: 0.1098039216, green: 0.1098039216, blue: 0.1098039216, alpha: 1) {
        didSet {
            self.setupUI()
        }
    }
...
extension CustomSwitch {
    ...
    public override func layoutSubviews() {
        ...
        if !self.isAnimating {
            ...
            let thumbSize = self.thumbSize != CGSize.zero ? self.thumbSize : CGSize(width: self.bounds.size.height - 5, height: self.bounds.height - 5)
            let yPostition = (self.bounds.size.height - thumbSize.height) / 2
            
            self.onPoint = CGPoint(x: (self.bounds.size.width - thumbSize.width - self.padding) - 3 , y: yPostition)
            self.offPoint = CGPoint(x: self.padding + 3 , y: yPostition)
